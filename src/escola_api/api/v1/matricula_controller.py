...
from src.escola_api.schemas.matricula_scemas import MatriculaCadastro, Matricula, MatriculaAluno
from src.escola_api.app import router


@router.get("/api/matriculas", status_code=200, tags=["matriculas"])
def listar_todas_matricula(id_curso: int = Query(alias="idCurso"), db: Session = Depends(get_db)):
...
def editar_matricula(id: int, form: Matricula, db: Session = Depends(get_db)):
    matricula = db.query(MatriculaEntidade).get(id)

    if matricula:
        matricula.curso_id = form.curso.id
        db.commit()
        db.refresh(matricula)
        return
    raise HTTPException(status_code=404, detail=f"Matricula não encontrada com id {id}")
